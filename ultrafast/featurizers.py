...
class Featurizer:
    ...
    def __call__(self, seq: str) -> torch.Tensor:
        if self.ext == ".lmdb" and self.db is not None:
            assert self.id_to_idx is not None, (
                "LMDB database must be preloaded to use this function"
            )
    ...
    def process_lmdb(self, seq_list: T.List[str]):
        ...
        db = px.Writer(
            dirpath=str(self.path), map_size_limit=self._map_size, ram_gb_limit=10
        )
        ...
        print(
            f"Processed and stored {len(sorted_ids)} {self.moltype} {'fingerprints' if self.moltype == 'drug' else 'sequences'} in LMDB."
        )
        ...
        **kwargs,
        ...
        if self._save_path.exists():
            if str(self._save_path).endswith(".h5"):
                ...
                self.id_to_idx = np.load(
                    self.path.parent / f"{self.name}_id_to_idx.npy", allow_pickle=True
                ).item()
...
class ChemGPTFeaturizer(Featurizer):
    def __init__(
        ...
        self.transformer = PretrainedHFTransformer(
            kind="Roberta-Zinc480M-102M",
            notation="selfies",
            dtype=float,
            device=self._device,
        )
    ...
    def _transform(self, batch_smiles: List[str]) -> torch.Tensor:
        try:
            ...
            if "CUDA out of memory" in str(e):
                print(
                    "CUDA out of memory during batch processing. Falling back to sequential processing."
                )
                return torch.stack(
                    [self._transform_single(smile) for smile in batch_smiles]
                )
            else:
                raise e
        except Exception as e:
...
class MorganFeaturizer(Featurizer):
    ...
    def smiles_to_morgan(self, smile: str):
        ...
        fpgen = rdFingerprintGenerator.GetMorganGenerator(
            radius=self._radius, fpSize=self.shape
        )
        try:
    ...
    def _transform(self, batch_smiles: List[str]) -> torch.Tensor:
        with multiprocessing.Pool(processes=self.n_jobs) as pool:
            ...
            all_feats = [
                torch.from_numpy(feat).squeeze().float()
                if feat.shape[0] == self.shape
                else torch.zeros(self.shape)
...
class ProtBertFeaturizer(Featurizer):
    ...
    def _transform(self, seqs: List[str]):
        ...
        embeddings = self._protbert_model(
...
class AMPLIFYFeaturizer(Featurizer):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            "AMPLIFY", shape, "target", save_dir, ext, batch_size, **kwargs
        )
...
class ESM2Featurizer(Featurizer):
    ...
    def _transform_single(self, seq: str) -> torch.Tensor:
        try:
            ...
            return token_embeddings[0, 1 : batch_lens - 1].squeeze(
                0
            )  # Return the full sequence embedding
        except Exception as e:
            print(f"Error featurizing single sequence: {seq}")
            return torch.zeros((len(seq), self.shape))  # zero vector for each token
    ...
    def _transform_single(self, seq: str) -> torch.Tensor:
        try:
            ...
            if isinstance(seq, str):
                return torch.zeros(
                    (len(seq) // 2, self.shape)
                )  # zero vector for each token
            else:
